...
import parse_dm3_grammar
parse_dm_header = parse_dm3_grammar.parse_dm3_header
...
def show_image(filepath, limits=None):
    ...
    def new_limits(low, high):
        ...
        canv._tkimage = canv.create_image(0, 1, anchor=NW, image=canv._photoimage)
        if canv._histimage:
            canv._histimage.destroy()
        canv._histimage = create_histogram(show_image.arr, root, 128, limits, new_limits)
    ...
    def next_image(step):
        if step != 0:
            ...
            names = [x for x in os.listdir(path) if os.path.splitext(x)[1] in [".dm3", ".dm4"]]
        ...
        show_image.arr = imagedatadict_to_ndarray(op['ImageList'][-1]['ImageData'])
...
def create_histogram(im, tkroot, size, limits, rangechangefunc):
    ...
    def add_hist_to_photoimage(im, out_pim, back="#D0D0D0", fore="#A0A0DD"):
        ...
        for j in range(hist_height,0,-1):
            ...
            pics += "{"+" ".join((back if x<lim else fore for x in vals))+"} "
        out_pim.put(pics)
    ...
    def start_click(e):
        ...
        clickstuff["box"] = canv_hist.create_rectangle((clickstuff["start"],0,clickstuff["start"],size))
    def move_click(e):
        if "box" in clickstuff:
            canv_hist.delete(clickstuff["box"])
            clickstuff["box"] = canv_hist.create_rectangle((clickstuff["start"],0,e.x,size))
    def end_click(e):
        if "box" in clickstuff:
            canv_hist.delete(clickstuff["box"])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2:
        ...
        print "Arguments are [Action] File. Action can be one of 'showimage', 'listdispersions', 'countdispersions', 'imageinfo'"
    if file:
        print file, action
        ...
        if action == "showimage":
            ...
            for kvlist in op["PrimaryList"]:
                print "Energy %lf has %d dispersions" % (kvlist['Prism']['Energy'], len(kvlist["DispersionList"]["List"]))
        elif action == "imageinfo":
            for i, image in enumerate(op["ImageList"]):
                print "Image %d: %s, PixelDepth: %d, DataType: %d (converted type %s)" % (
